...
import { btcFormatter, COMP, logTxDetails, mBTC, fUSD, stkAAVE, USDC, usdFormatter, USDT } from "./utils"
...
subtask("collect-interest-dist", "Collects and distributes fAsset interest").setAction(async (taskArgs, hre) => {
    const signer = await getSigner(hre, taskArgs.speed)
    const chain = getChain(hre)

    const collector = Collector__factory.connect(resolveAddress("Collector", chain), signer)

    const tx = await collector.distributeInterest([fUSD.address, mBTC.address], false)
    const receipt = await logTxDetails(tx, `collect fees from fUSD and mBTC`)
    ...
    const fusdEvent = parsedEvents.find((e) => e.name === "RevenueRedistributed" && e.args.fAsset === fUSD.address)
    console.log(`fUSD revenue: ${usdFormatter(fusdEvent.args.amount)}`)
    const mbtcEvent = parsedEvents.find((e) => e.name === "RevenueRedistributed" && e.args.fAsset === mBTC.address)
