...
describe("Feeder - Mint", () => {
    ...
    const assertBasicMint = async (
        ...
        // If not a main pool asset, so only a fdAsset or mAsset
        ...
        if (!assetBefore.isMpAsset && platformInteraction.expectInteraction) {
            ...
            // TODO this check is not working for mAssets or fdAssets using a lending market
    ...
    describe("minting with a single asset", () => {
        ...
        context("when the basket is initially 75% mAsset and 25% fdAsset and then f is drastically scaled by the redemption price", () => {
            beforeEach(async () => {
            ...
            it("set RP to 3 and mint with mAsset", async () => {
                ...
                // fdAsset is now worth 3 times as much as mAsset so total value in pool has increased by 50%. Now adding
        ...
        context("when the basket is initially 33% mAsset and 67% fdAsset and then RP drops to half", () => {
            beforeEach(async () => {
            ...
            it("set RP to 0.5 and mint with mAsset", async () => {
                ...
                // mAsset is worth double fdAsset and TVL has dropped by a third, so expect 1 mAsset to return 1 / (2/3)
        ...
        context("when the basket is 75% mAsset, 25% fdAsset", () => {
            beforeEach(async () => {
            ...
            context("mAsset is overweight", () => {
                ...
                it("should fail to mint fdAsset if mAsset is still overweight", async () => {
                    await assertFailedMint("Exceeds weight limits", details.pool, details.mAsset, simpleToExactAmount(1))
                })
                it("should mint fdAsset so mAsset is underweight", async () => {
    ...
    describe("minting with multiple bAssets", () => {
        context("when the weights are within the ForgeValidator limit", () => {
            context("passing invalid arguments", async () => {
                ...
                context("when all quantities are zero", () => {
                    it("should fail to mint fdAsset and mAsset", async () => {
                        await assertFailedMintMulti("Zero mAsset quantity", details.pool, details.bAssets, [0, 0], 0)
                    })
                    it("should fail to mint feeder asset", async () => {
                        await assertFailedMintMulti("Zero mAsset quantity", details.pool, [details.fdAsset], [0], 0)
                    })
                    it("should fail to mint mStable asset", async () => {
        ...
        context("when the basket is 21% mAsset, 79% fdAsset", () => {
            beforeEach(async () => {
                await runSetup(false, false, [21, 79])
            })
            it("should fail to multi mint the smallest unit of fdAsset", async () => {
                await assertFailedMintMulti("Zero mAsset quantity", details.pool, [details.fdAsset], [1], 0, 0, true)
            })
            ...
            context("fdAsset is overweight", () => {
                ...
                it("should fail to multi mint mAsset if fdAsset is still overweight", async () => {
                    await assertFailedMintMulti(
                        "Exceeds weight limits",
                        details.pool,
                        [details.fdAsset],
                ...
                it("should mint mAsset so fdAsset is underweight", async () => {
