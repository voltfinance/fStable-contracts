...
describe("SavingsManager", async () => {
    ...
    async function createNewSavingsManager(mintAmount: BN = INITIAL_MINT): Promise<void> {
        fUSD = await (await new MockFasset__factory(sa.default.signer)).deploy("MOCK", "MOCK", 18, sa.default.address, mintAmount)
        ...
        savingsContract = await savingsFactory.deploy(nexus.address, fUSD.address, unwrapperContract.address)
        await savingsContract.initialize(sa.default.address, "Savings Credit", "ifUSD")
        ...
        await fUSD.connect(sa.default.signer).transfer(liquidator.address, simpleToExactAmount(1, 23))
    }
    ...
    describe("constructor", async () => {
        ...
        it("should fail when fAsset address is zero", async () => {
            await expect(
                new SavingsManager__factory(sa.default.signer).deploy(
                    nexus.address,
                    [ZERO_ADDRESS],
        ...
        it("should have valid state after deployment", async () => {
            const savingsContractAddr = await savingsManager.savingsContracts(fUSD.address)
            expect(savingsContractAddr).to.equal(savingsContract.address)

            const allowance = await fUSD.allowance(savingsManager.address, savingsContract.address)
            expect(MAX_UINT256).to.equal(allowance)
        })
    })

    describe("adding a SavingsContract", async () => {
        let mockFasset: MockERC20
        let mockSavingsContract: Account

        before(async () => {
            mockSavingsContract = sa.dummy4
            mockFasset = await new MockFasset__factory(sa.default.signer).deploy("MOCK", "MOCK", 18, sa.default.address, BN.from(10000))
        })

        it("should fail when not called by governor", async () => {
            await expect(
                savingsManager.connect(sa.other.signer).addSavingsContract(mockFasset.address, mockSavingsContract.address),
        ...
        it("should fail when savingsContract address is zero", async () => {
            await expect(
                savingsManager.connect(sa.governor.signer).addSavingsContract(mockFasset.address, ZERO_ADDRESS),
            ).to.be.revertedWith("Must be valid address")
        })

        it("should fail when fAsset entry already exist", async () => {
            await expect(
                savingsManager.connect(sa.governor.signer).addSavingsContract(fUSD.address, savingsContract.address),
            ).to.be.revertedWith("Savings contract already exists")
        })

        it("should succeed with valid parameter", async () => {
            let savingsContractAddr = await savingsManager.savingsContracts(fUSD.address)
            expect(savingsContractAddr).to.equal(savingsContract.address)
            savingsContractAddr = await savingsManager.savingsContracts(mockFasset.address)
            expect(ZERO_ADDRESS).to.equal(savingsContractAddr)
            const tx = savingsManager.connect(sa.governor.signer).addSavingsContract(mockFasset.address, mockSavingsContract.address)
            await expect(tx).to.emit(savingsManager, "SavingsContractAdded").withArgs(mockFasset.address, mockSavingsContract.address)

            savingsContractAddr = await savingsManager.savingsContracts(fUSD.address)
            expect(savingsContractAddr).to.equal(savingsContract.address)
    ...
    describe("updating a SavingsContract", async () => {
        it("should fail when not called by governor", async () => {
            await expect(
                savingsManager.connect(sa.other.signer).updateSavingsContract(fUSD.address, savingsContract.address),
            ).to.be.revertedWith("Only governor can execute")
        })
        ...
        it("should fail when savingsContract address is zero", async () => {
            await expect(savingsManager.connect(sa.governor.signer).updateSavingsContract(fUSD.address, ZERO_ADDRESS)).to.be.revertedWith(
        ...
        it("should succeed with valid parameters", async () => {
            ...
            const tx = savingsManager.connect(sa.governor.signer).updateSavingsContract(fUSD.address, sa.other.address)

            await expect(tx).to.emit(savingsManager, "SavingsContractUpdated").withArgs(fUSD.address, sa.other.address)
    ...
    describe("freezing streams", async () => {
        ...
        it("should stop all streaming from being initialized", async () => {
            ...
            await expect(savingsManager.collectAndStreamInterest(fUSD.address)).to.be.revertedWith("Streaming is currently frozen")
        })
    })

    describe("adding a revenue recipient", async () => {
        it("should fail when not called by governor", async () => {
            await expect(savingsManager.connect(sa.other.signer).setRevenueRecipient(fUSD.address, DEAD_ADDRESS)).to.be.revertedWith(
                "Only governor can execute",
            )
        })
        it("should simply update the recipient and emit an event", async () => {
            const tx = savingsManager.connect(sa.governor.signer).setRevenueRecipient(fUSD.address, sa.fundManager.address)
            await expect(tx).to.emit(savingsManager, "RevenueRecipientSet").withArgs(fUSD.address, sa.fundManager.address)
            const recipient = await savingsManager.revenueRecipients(fUSD.address)
    ...
    describe("collecting and distributing Interest", async () => {
        ...
        context("with invalid arguments", async () => {
            it("should fail when fAsset not exist", async () => {
        ...
        context("when the contract is paused", async () => {
            it("should fail", async () => {
                ...
                await expect(savingsManager.collectAndDistributeInterest(fUSD.address)).to.be.revertedWith("Pausable: paused")
            })
        })
        context("when there is no interest to collect", async () => {
            ...
            it("should succeed when interest collected is zero", async () => {
                const tx = savingsManager.collectAndDistributeInterest(fUSD.address)
                await expect(tx)
                    .to.emit(savingsManager, "InterestCollected")
                    .withArgs(fUSD.address, BN.from(0), INITIAL_MINT.mul(BN.from(10).pow(BN.from(18))), BN.from(0))
            })
        })
        ...
        const snapshotData = async (): Promise<Data> => {
            const liqStream = await savingsManager.liqStream(fUSD.address)
            const yieldStream = await savingsManager.yieldStream(fUSD.address)
            return {
                lastPeriodStart: await savingsManager.lastPeriodStart(fUSD.address),
                lastCollection: await savingsManager.lastCollection(fUSD.address),
                periodYield: await savingsManager.periodYield(fUSD.address),
                liqStream: { end: liqStream[0], rate: liqStream[1] },
                yieldStream: { end: yieldStream[0], rate: yieldStream[1] },
                savingsManagerBal: await fUSD.balanceOf(savingsManager.address),
                savingsContractBal: await fUSD.balanceOf(savingsContract.address),
                lastBatchCollected: await savingsManager.lastBatchCollected(fUSD.address),
            }
        }
        context("testing the boundaries of liquidated deposits", async () => {
            ...
            it("should fail if deposit not called by the liquidator", async () => {
                await expect(savingsManager.connect(sa.dummy2.signer).depositLiquidation(fUSD.address, liquidated1)).to.be.revertedWith(
                    "Only liquidator can execute",
                )
            })
            it("should fail if sender has no fUSD approved", async () => {
                await expect(savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidated1)).to.be.revertedWith(
                    "ERC20: transfer amount exceeds allowance",
                )
            })
            it("should set the streamRate and finish time correctly", async () => {
                const before = await snapshotData()
                await fUSD.connect(liquidator.signer).approve(savingsManager.address, liquidated1)

                const tx = savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidated1)
                await expect(tx).to.emit(savingsManager, "LiquidatorDeposited").withArgs(fUSD.address, liquidated1)
            ...
            it("should work over multiple periods", async () => {
                ...
                await fUSD.connect(liquidator.signer).approve(savingsManager.address, liquidated1)
                await savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidated1)
                ...
                await fUSD.setAmountForCollectInterest(1)
                ...
                await fUSD.connect(liquidator.signer).approve(savingsManager.address, liquidated2)
                await savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidated2)
                ...
                // @18
                await fUSD.connect(liquidator.signer).approve(savingsManager.address, liquidated3)
                await savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidated3)
                const s18 = await snapshotData()
                assertBNClosePercent(s18.liqStream.rate, liquidated3.div(ONE_WEEK), "0.001")
            })
        })

        context("testing the collection and streaming of fAsset interest", async () => {
            // Initial supply of 10m units
            const initialSupply = BN.from(10000000)
            const liquidated1 = simpleToExactAmount(100, 18)
            ...
            it("should fail if called twice within 6 hours", async () => {
                await fUSD.setAmountForPlatformInterest(BN.from(10000))
                await savingsManager.collectAndStreamInterest(fUSD.address)
                await expect(savingsManager.connect(sa.dummy2.signer).collectAndStreamInterest(fUSD.address)).to.be.revertedWith(
                    "Cannot deposit twice in 6 hours",
                )
            })
            it("should have no effect if there is no interest to collect", async () => {
                const before = await snapshotData()
                const bal = await fUSD.totalSupply()
                const tx = savingsManager.collectAndStreamInterest(fUSD.address)
                await expect(tx).to.emit(savingsManager, "InterestCollected").withArgs(fUSD.address, BN.from(0), bal, BN.from(0))
            ...
            it("should fail if the APY is too high", async () => {
                ...
                // initial liq = 10m
                // 12h increase = ~~205k
                await fUSD.setAmountForPlatformInterest(simpleToExactAmount(210000, 18))
                await expect(savingsManager.connect(sa.dummy2.signer).collectAndStreamInterest(fUSD.address)).to.be.revertedWith(
                    "Interest protected from inflating past maxAPY",
                )
                await fUSD.setAmountForPlatformInterest(simpleToExactAmount(200000, 18))
                const tx = savingsManager.collectAndStreamInterest(fUSD.address)
                await expect(tx).to.emit(savingsManager, "InterestCollected")
            })
            it("should factor in new fUSD, initialise stream and emit an event", async () => {
                const before = await snapshotData()
                ...
                await fUSD.setAmountForPlatformInterest(collectionAmount)
                await savingsManager.collectAndStreamInterest(fUSD.address)
            ...
            it("should coexist with liquidator stream to allow simultaneous streaming", async () => {
                ...
                // @1
                await fUSD.setAmountForPlatformInterest(platformInterest1)
                await savingsManager.collectAndStreamInterest(fUSD.address)
                ...
                await fUSD.setAmountForPlatformInterest(platformInterest2)
                await savingsManager.collectAndStreamInterest(fUSD.address)
                ...
                await fUSD.setAmountForPlatformInterest(platformInterest3)
                await savingsManager.collectAndStreamInterest(fUSD.address)
                ...
                // all fUSD should be drained now
                expect(s16.savingsManagerBal).lt(simpleToExactAmount(1, 16))
                await increaseTime(ONE_DAY.div(2))
                // @16.5
                const ts17 = await getTimestamp()
                await fUSD.setAmountForPlatformInterest(platformInterest4)
                await savingsManager.collectAndStreamInterest(fUSD.address)
        ...
        context("testing new mechanism", async () => {
            ...
            it("should work when lastCollection time is 0 with low interest", async () => {
                const lastPeriodStart = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart).eq(BN.from(0))
                const lastCollection = await savingsManager.lastCollection(fUSD.address)
                expect(lastCollection).eq(BN.from(0))
                const lastPeriodYield = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(newInterest)

                const curTime = await getTimestamp()
                const tx = await savingsManager.collectAndDistributeInterest(fUSD.address)
                ...
                const lastCollectionAfter = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollectionAfter, curTime, BN.from(2))
                const lastPeriodStartAfter = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStartAfter).eq(lastCollectionAfter)
                const lastPeriodYieldAfter = await savingsManager.periodYield(fUSD.address)
            ...
            it("should always update the lastCollection time for future", async () => {
                ...
                const lastCollectionMiddle = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollectionMiddle, curTime, BN.from(2))

                const newInterest = simpleToExactAmount(11000, 18)
                await fUSD.setAmountForCollectInterest(newInterest)

                await expect(savingsManager.collectAndDistributeInterest(fUSD.address)).to.be.revertedWith(
                ...
                const lastCollectionEnd = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollectionEnd, endTime, BN.from(2))
                const lastPeriodStartEnd = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStartEnd).eq(lastCollectionEnd)
                const lastPeriodYieldEnd = await savingsManager.periodYield(fUSD.address)
            ...
            it("should fail if under 30 minutes and greater than 10bps increase", async () => {
                ...
                await fUSD.setAmountForCollectInterest(failingInterest)
                await expect(savingsManager.collectAndDistributeInterest(fUSD.address)).to.be.revertedWith(
                    "Interest protected from inflating past 10 Bps",
                )
                // change to 9.99bps
                const passingInterest = simpleToExactAmount(9999, 18)
                await fUSD.setAmountForCollectInterest(passingInterest)
            ...
            it("should pass if 1 block and 9.99bps increase", async () => {
                ...
                const lastCollectionBefore = await savingsManager.lastCollection(fUSD.address)
            ...
            it("should updated period information correctly across sequence", async () => {
                ...
                // @0
                const lastCollection_0 = await savingsManager.lastCollection(fUSD.address)
                expect(lastCollection_0).eq(BN.from(0))
                const lastPeriodStart_0 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_0).eq(BN.from(0))
                const periodYield_0 = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(interest_40)
                await savingsManager.collectAndDistributeInterest(fUSD.address)
                const lastCollection_40 = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollection_40, curTime, 5)
                const lastPeriodStart_40 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_40).eq(lastCollection_40)
                const periodYield_40 = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(interest_50)
                await savingsManager.collectAndDistributeInterest(fUSD.address)
                const lastCollection_50 = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollection_50, curTime, 5)
                const lastPeriodStart_50 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_50).eq(lastCollection_40)
                const periodYield_50 = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(interest_65)
                await savingsManager.collectAndDistributeInterest(fUSD.address)
                const lastCollection_65 = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollection_65, curTime, 5)
                const lastPeriodStart_65 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_65).eq(lastCollection_40)
                const periodYield_65 = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(interest_80)
                await savingsManager.collectAndDistributeInterest(fUSD.address)
                const lastCollection_80 = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollection_80, curTime, 5)
                const lastPeriodStart_80 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_80).eq(lastCollection_65)
                const periodYield_80 = await savingsManager.periodYield(fUSD.address)
                ...
                await fUSD.setAmountForCollectInterest(interest_120)
                await savingsManager.collectAndDistributeInterest(fUSD.address)
                const lastCollection_120 = await savingsManager.lastCollection(fUSD.address)
                assertBNClose(lastCollection_120, curTime, 5)
                const lastPeriodStart_120 = await savingsManager.lastPeriodStart(fUSD.address)
                expect(lastPeriodStart_120).eq(lastCollection_120)
                const periodYield_120 = await savingsManager.periodYield(fUSD.address)
        ...
        context("when there is some interest to collect", async () => {
            ...
            it("should collect the interest first time", async () => {
                ...
                const balanceBefore = await fUSD.balanceOf(savingsContract.address)
                ...
                expect(eventArgs.fAsset).eq(fUSD.address)
                ...
                const balanceAfter = await fUSD.balanceOf(savingsContract.address)
            ...
            it("should allow interest collection again after 30 mins", async () => {
                ...
                await expect(tx)
                    .to.emit(savingsManager, "InterestCollected")
                    .withArgs(fUSD.address, BN.from(0), INITIAL_MINT.mul(fullScale), BN.from(0))
            })
        })
    })
    ...
    describe("distributing unallocated Interest", async () => {
        it("should fail without a valid recipient", async () => {
            await expect(savingsManager.connect(sa.governor.signer).distributeUnallocatedInterest(fUSD.address)).to.be.revertedWith(
                "Must have valid recipient",
            )
        })

        it("calls the distribute function on a valid recipient", async () => {
            const balanceBefore = await fUSD.balanceOf(sa.other.address)
            expect(ZERO).to.equal(balanceBefore)

            // Send some fUSD to SavingsManager
            const amount = BN.from(1000)
            await fUSD.connect(sa.default.signer).transfer(savingsManager.address, amount)

            const recipient = await new MockRevenueRecipient__factory(sa.default.signer).deploy()
            await savingsManager.connect(sa.governor.signer).setRevenueRecipient(fUSD.address, recipient.address)
            const tx = savingsManager.connect(sa.governor.signer).distributeUnallocatedInterest(fUSD.address)

            await expect(tx).to.emit(savingsManager, "RevenueRedistributed").withArgs(fUSD.address, recipient.address, amount)

            const balanceAfter = await fUSD.balanceOf(recipient.address)
        ...
        it("calculates the unallocated interest correctly and calls the recipient", async () => {
            ...
            await fUSD.connect(liquidator.signer).approve(savingsManager.address, liquidationAmount)
            await savingsManager.connect(liquidator.signer).depositLiquidation(fUSD.address, liquidationAmount)
            // Zoom forward 3 days
            await increaseTime(ONE_DAY.mul(3))
            // Set interest for collection
            await fUSD.setAmountForCollectInterest(swapFeesAmount)
            // Deposit to SAVE
            await savingsManager.collectAndDistributeInterest(fUSD.address)
            ...
            await savingsManager.connect(sa.governor.signer).distributeUnallocatedInterest(fUSD.address)
            const balance00 = await fUSD.balanceOf(recipient.address)
            assertBNClosePercent(expectedRedistribution, balance00, "0.01")
            // Zoom forward 1 days
            await increaseTime(ONE_DAY)
            // Redistribution should net 0
            await savingsManager.connect(sa.governor.signer).distributeUnallocatedInterest(fUSD.address)
            const balance01 = await fUSD.balanceOf(recipient.address)
    ...
    describe("extra tests:", async () => {
        ...
        it("should collect when 0% unallocated interest", async () => {
            ...
            let savingsManagerBalance = await fUSD.balanceOf(savingsManager.address)
            ...
            savingsManagerBalance = await fUSD.balanceOf(savingsManager.address)
            expect(ZERO).to.equal(savingsManagerBalance)
        })
        ...
        it("should collect 10% unallocated interest when rate changed", async () => {
            ...
            await fUSD.setAmountForCollectInterest(FIVE_TOKENS)
            ...
            const balanceOfRecipient = await fUSD.balanceOf(recipient.address)
