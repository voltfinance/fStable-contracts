...
import { DAI, mBTC, MTA, fUSD, USDC, WBTC } from "tasks/utils/tokens"
import {
    EmissionsController,
...
describe("Fork test deploy of RevenueSplitBuyBack", async () => {
    ...
    describe("Next revenue buy back", () => {
        ...
        it("Configure RevenueSplitBuyBack", async () => {
            await revenueBuyBack.connect(governor).mapBasset(fUSD.address, USDC.address)
        ...
        context("buy back MTA using fUSD and mBTC", () => {
            ...
            it("Buy back MTA using fUSD", async () => {
                const fusdRbbBalBefore = await fusdToken.balanceOf(revenueBuyBack.address)
                expect(fusdRbbBalBefore, "fUSD bal before").to.gt(0)
                ...
                const minBassets = fusdRbbBalBefore.mul(99).div(100).div(2).div(1e12)
                console.log(`minBassets ${minBassets} = ${fusdRbbBalBefore} * 98% / 1e12`)
                // MTA = USD * MTA/USD price * 10^(18-6) to convert from 6 to 18 decimals
                const minMta = minBassets.mul(mtaUsdPrice).div(100).mul(1e12)
                await revenueBuyBack.buyBackRewards([fUSD.address], [minBassets], [minMta], [fusdUniswapPath.encoded])
