...
struct RevenueBuyBackConfig {
    // Minimum price of bAssets compared to fAssets scaled to 1e18 (CONFIG_SCALE).
    uint128 minFasset2BassetPrice;
    // Minimum price of rewards token compared to bAssets scaled to 1e18 (CONFIG_SCALE).
    uint128 minBasset2RewardsPrice;
    // base asset of the fAsset that is being redeemed and then sold for reward tokens.
    address bAsset;
    // Uniswap V3 path
    bytes uniswapPath;
}
...
contract RevenueBuyBack is IRevenueRecipient, Initializable, ImmutableModule {
    ...
    /// @notice Mapping of fAssets to RevenueBuyBack config
    mapping(address => RevenueBuyBackConfig) public fassetConfig;
    ...
    ****************************************/
    ...
    function notifyRedistributionAmount(address _fAsset, uint256 _amount) external override {
        require(fassetConfig[_fAsset].bAsset != address(0), "Invalid fAsset");
    ...
    /**
     * @notice Buys reward tokens, eg MTA, using fAssets like fUSD or mBTC from protocol revenue.
     * @param _fAssets Addresses of fAssets that are to be sold for rewards. eg fUSD and mBTC.
     */
    function buyBackRewards(address[] calldata _fAssets) external onlyKeeperOrGovernor {
        uint256 len = _fAssets.length;
        require(len > 0, "Invalid args");

        // for each fAsset
        for (uint256 i = 0; i < len; i++) {
            // Get config for fAsset
            RevenueBuyBackConfig memory config = fassetConfig[_fAssets[i]];
            require(config.bAsset != address(0), "Invalid fAsset");

            // STEP 1 - Redeem fAssets for bAssets
            IFasset fAsset = IFasset(_fAssets[i]);
            uint256 fAssetBal = IERC20(_fAssets[i]).balanceOf(address(this));
            uint256 minBassetOutput = (fAssetBal * config.minFasset2BassetPrice) / CONFIG_SCALE;
            uint256 bAssetAmount = fAsset.redeem(
                config.bAsset,
                fAssetBal,
                minBassetOutput,
                address(this)
            ...
            emit BuyBackRewards(_fAssets[i], fAssetBal, bAssetAmount, rewardsAmount);
        }
    }
    ...
    ****************************************/
    ...
    /**
     * @notice Adds or updates rewards buyback config for a fAsset.
     * @param _fAsset Address of the meta asset that is received as protocol revenue.
     * @param _bAsset Address of the base asset that is redeemed from the fAsset.
     * @param _minFasset2BassetPrice Minimum price of bAssets compared to fAssets scaled to 1e18 (CONFIG_SCALE).
     * eg USDC/fUSD and wBTC/mBTC exchange rates.
     * USDC has 6 decimal places so `minFasset2BassetPrice` with no slippage is 1e6.
     * If a 2% slippage is allowed, the `minFasset2BassetPrice` is 98e4.
     * WBTC has 8 decimal places so `minFasset2BassetPrice` with no slippage is 1e8.
     * If a 5% slippage is allowed, the `minFasset2BassetPrice` is 95e6.
     ...
     */
    function setFassetConfig(
        address _fAsset,
        address _bAsset,
        uint128 _minFasset2BassetPrice,
        ...
        require(_minFasset2BassetPrice > 0, "Invalid min bAsset price");
        ...
        fassetConfig[_fAsset] = RevenueBuyBackConfig({
            bAsset: _bAsset,
            minFasset2BassetPrice: _minFasset2BassetPrice,
            minBasset2RewardsPrice: _minBasset2RewardsPrice,
            uniswapPath: _uniswapPath
        });

        emit AddedFassetConfig(
            _fAsset,
            _bAsset,
            _minFasset2BassetPrice,
