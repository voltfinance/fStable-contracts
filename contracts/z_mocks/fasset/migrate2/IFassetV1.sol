...
/**
 * @title fUSD interface before upgrade to V3
 */
interface IFassetV1 {
    /** @dev Calc interest */
    ...
    /** @dev Minting */
    function mint(address _basset, uint256 _bassetQuantity) external returns (uint256 fassetMinted);

    function mintTo(
        address _basset,
        uint256 _bassetQuantity,
        address _recipient
    ) external returns (uint256 fassetMinted);
    ...
    function redeem(address _basset, uint256 _bassetQuantity)
        external
        returns (uint256 fassetRedeemed);

    function redeemTo(
        address _basset,
        uint256 _bassetQuantity,
        address _recipient
    ) external returns (uint256 fassetRedeemed);
    ...
    function redeemFasset(uint256 _fAssetQuantity, address _recipient) external;
