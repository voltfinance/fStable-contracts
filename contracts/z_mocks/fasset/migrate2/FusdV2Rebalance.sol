...
import { IFassetV1 } from "./IFassetV1.sol";
import { DyDxFlashLoan } from "../../../peripheral/dydx/DyDxFlashLoan.sol";
import { ICurve } from "../../../peripheral/Curve/ICurve.sol";

/**
 * @title   Contract to rebalance fUSD bAssets to new weights for the fUSD V3 upgrade.
 * @author  mStable
 * @notice  Either DAI or USDC is flash loaned from DyDx to swap for TUSD or USDT in fUSD.
 ...
 *          DATE:    2021-03-22
 */
contract FusdV2Rebalance is DyDxFlashLoan, Ownable {
    using SafeERC20 for IERC20;

    // Contracts that are called to execute swaps
    IFassetV1 constant fUsdV1 = IFassetV1(0xe2f2a5C287993345a840Db3B0845fbC70f5935a5);
    ...
    ****************************************/
    ...
    /**
     * @notice Convert TUSD in the fUSD basket to USDC or DAI using a DyDx flash loan.
    ...
    /**
     * @notice Convert TUSD and USDT in the fUSD basket to USDC or DAI using a DyDx flash loan.
     * @param flashToken DAI or USDC token address.
     * @param funderAccount Account that will fund the shortfall of the DyDx USDC flash loan.
     * @param swapInputs This fUSD swap inputs from the flash token to TUSD (at index 0) and USDT (at index 1).
     * @dev Assumes the funder has already approved this contract to transferFrom the shortfall from their account.
     */
    ...
    ****************************************/
    ...
    /**
     * @notice Executes the following swaps to rebalance the fUSD bAssets:
        Swap flash token for TUSD using fUSD
        Swap flash token for USDT using fUSD
    ...
    function _balanceTusdAndUsdt(
        ...
        uint256 flashAmount = swapInputs[0] + swapInputs[1];
        // Approve fUSD contract to transfer flash token from this contract
        // console.log("About to approve fUSD contract to transfer %s flash tokens >= %s %s", flashAmount, swapInputs[0], swapInputs[1]);
        require(flashAmount >= swapInputs[0] + swapInputs[1], "flash loan not >= swap inputs");
        IERC20(flashToken).safeApprove(address(fUsdV1), flashAmount);

        // If swapping flash token into fUSD for TUSD
        if (swapInputs[0] > 0) {
            // Swap flash token for TUSD using fUSD
            // console.log("About to fUSD swap %s flash tokens for TUSD", swapInputs[0]);
            uint256 tusdOutput = fUsdV1.swap(flashToken, TUSD, swapInputs[0], address(this));
        ...
        // If swapping flash token into fUSD for USDT
        if (swapInputs[1] > 0) {
            // Swap flash token for USDT using fUSD
            // console.log("About to fUSD swap %s flash tokens for USDT", swapInputs[1]);
            uint256 usdtOutput = fUsdV1.swap(flashToken, USDT, swapInputs[1], address(this));
