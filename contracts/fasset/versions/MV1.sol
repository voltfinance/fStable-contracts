...
import "../FassetStructs.sol";

// Libs
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { StableMath } from "../../shared/StableMath.sol";
import { FassetManager } from "../FassetManager.sol";
import { FassetLogic } from "../FassetLogic.sol";
import { MV1Migrator } from "./MV1Migrator.sol";

// Legacy
import { IBasketManager } from "../../z_mocks/fasset/migrate2/IBasketManager.sol";
import { Basket, Basset } from "../../z_mocks/fasset/migrate2/FassetStructsV1.sol";
import { InitializableModuleV1 } from "../../z_mocks/fasset/migrate2/InitializableModuleV1.sol";

/**
 * @title   Fasset used to migrate fUSD from V2.0 to V3.0
 * @author  mStable
 ...
 *          stablecoin (fAsset) and redirects lending market interest and swap fees to the savings
    ...
    ImmutableModule
{
    ...
    // Deprecated - maintain for storage layout in fUSD
     ...
     * @param _nameArg          Name of the fAsset
     * @param _symbolArg        Symbol of the fAsset
    ...
    ****************************************/
     ...
     * @param _input             Address of the bAsset to deposit for the minted fAsset.
     * @param _inputQuantity     Quantity in bAsset units
     * @param _minOutputQuantity Minimum fAsset quanity to be minted. This protects against slippage.
     * @param _recipient         Receipient of the newly minted fAsset tokens
     * @return mintOutput        Quantity of newly minted fAssets for the deposited bAsset.
     */
    function mint(
        address _input,
    ...
    /**
     * @dev Mint with multiple bAssets, at a 1:1 ratio to fAsset. This contract
     *      must have approval to spend the senders bAssets
     * @param _inputs            Non-duplicate address array of bASset addresses to deposit for the minted fAsset tokens.
     * @param _inputQuantities   Quantity of each bAsset to deposit for the minted fAsset.
     *                           Order of array should mirror the above bAsset addresses.
     * @param _minOutputQuantity Minimum fAsset quanity to be minted. This protects against slippage.
     * @param _recipient         Address to receive the newly minted fAsset tokens
     * @return mintOutput    Quantity of newly minted fAssets for the deposited bAssets.
     */
    function mintMulti(
        ...
        mintOutput = FassetLogic.mintMulti(
            data,
            _getConfig(),
            indexes,
            _inputQuantities,
     ...
     * @param _input             Address of the bAsset to deposit for the minted fAsset
     * @param _inputQuantity     Quantity in bAsset units
     * @return mintOutput        Estimated mint output in fAsset terms
     */
        ...
        returns (uint256 mintOutput)
    {
        ...
        mintOutput = FassetLogic.computeMint(
            data.bAssetData,
            input.idx,
            _inputQuantity,
            _getConfig()
    ...
    /**
     * @dev Get the projected output of a given mint
     * @param _inputs            Non-duplicate address array of addresses to bAssets to deposit for the minted fAsset tokens.
     * @param _inputQuantities  Quantity of each bAsset to deposit for the minted fAsset.
     * @return mintOutput        Estimated mint output in fAsset terms
     */
        ...
        returns (uint256 mintOutput)
    {
        ...
        uint8[] memory indexes = _getAssets(_inputs);
        return
            FassetLogic.computeMintMulti(data.bAssetData, indexes, _inputQuantities, _getConfig());
    }
    ...
    ****************************************/
    ...
    function swap(
        ...
        uint256 scaledFee;
        (swapOutput, scaledFee) = FassetLogic.swap(
            data,
            _getConfig(),
            input,
            output,
    ...
    function getSwapOutput(
        ...
        (swapOutput, ) = FassetLogic.computeSwap(
            data.bAssetData,
            input.idx,
            output.idx,
            _inputQuantity,
    ...
    ****************************************/
    ...
    /**
     * @notice Redeems a specified quantity of fAsset in return for a bAsset specified by bAsset address.
     * The bAsset is sent to the specified recipient.
     * The bAsset quantity is relative to current vault balance levels and desired fAsset quantity.
     * The quantity of fAsset is burnt as payment.
     * A minimum quantity of bAsset is specified to protect against price slippage between the fAsset and bAsset.
     * @param _output            Address of the bAsset to receive
     * @param _fAssetQuantity    Quantity of fAsset to redeem
     * @param _minOutputQuantity Minimum bAsset quantity to receive for the burnt fAssets. This protects against slippage.
     * @param _recipient         Address to transfer the withdrawn bAssets to.
     * @return outputQuantity    Quanity of bAsset units received for the burnt fAssets
     */
    function redeem(
        address _output,
        ...
        require(_recipient != address(0), "Invalid recipient");
        require(_fAssetQuantity > 0, "Qty==0");

        Asset memory output = _getAsset(_output);

        // Get config before burning. Config > Burn > CacheSize
        InvariantConfig memory config = _getConfig();
        _burn(msg.sender, _fAssetQuantity);

        uint256 scaledFee;
        (outputQuantity, scaledFee) = FassetLogic.redeem(
            data,
            config,
            output,
            _fAssetQuantity,
     ...
     * balance levels and desired fAsset quantity. Burns the fAsset as payment.
     * @param _fAssetQuantity       Quantity of fAsset to redeem
    ...
    function redeemFasset(
        ...
        uint256 scaledFee;
        (scaledFee, outputs, outputQuantities) = FassetLogic.redeemProportionately(
            data,
            config,
            _fAssetQuantity,
     ...
     *      relative Fasset quantity from the sender. Sender also incurs a small fee on the outgoing asset.
     * @param _outputs           Addresses of the bAssets to receive
     * @param _outputQuantities  Units of the bAssets to redeem
     * @param _maxFassetQuantity Maximum fAsset quantity to burn for the received bAssets. This protects against slippage.
     * @param _recipient         Address to receive the withdrawn bAssets
     * @return fAssetQuantity    Quantity of fAsset units burned plus the swap fee to pay for the redeemed bAssets
     */
    function redeemExactBassets(
        ...
        address _recipient
    ) external override nonReentrant whenNoRecol returns (uint256 fAssetQuantity) {
        require(_recipient != address(0), "Invalid recipient");
        uint256 len = _outputQuantities.length;
        require(len > 0 && len == _outputs.length, "Invalid array input");
        require(_maxFassetQuantity > 0, "Qty==0");

        uint8[] memory indexes = _getAssets(_outputs);

        uint256 fee;
        (fAssetQuantity, fee) = FassetLogic.redeemExactBassets(
            data,
            _getConfig(),
            indexes,
            _outputQuantities,
        ...
        _burn(msg.sender, fAssetQuantity);
     ...
     * @param _output            Address of the bAsset to receive
     * @param _fAssetQuantity    Quantity of fAsset to redeem
     * @return bAssetOutput      Estimated quantity of bAsset units received for the burnt fAssets
     */
    function getRedeemOutput(address _output, uint256 _fAssetQuantity)
        external
        view
        override
        returns (uint256 bAssetOutput)
    {
        ...
        (bAssetOutput, ) = FassetLogic.computeRedeem(
            data.bAssetData,
            output.idx,
            _fAssetQuantity,
            _getConfig(),
     ...
     * @return fAssetQuantity    Estimated quantity of fAsset units needed to burn to receive output
     */
    function getRedeemExactBassetsOutput(
        address[] calldata _outputs,
        uint256[] calldata _outputQuantities
    ) external view override returns (uint256 fAssetQuantity) {
        uint256 len = _outputQuantities.length;
        require(len > 0 && len == _outputs.length, "Invalid array input");

        uint8[] memory indexes = _getAssets(_outputs);

        // calculate the value of fAssets need to cover the value of bAssets being redeemed
        (fAssetQuantity, ) = FassetLogic.computeRedeemExact(
            data.bAssetData,
            indexes,
            _outputQuantities,
            _getConfig(),
    ...
    ****************************************/
    ...
    /**
     * @dev Get basket details for `Fasset_FassetStructs.Basket`
    ...
    function getPrice() external view override returns (uint256 price, uint256 k) {
        return FassetLogic.computePrice(data.bAssetData, _getConfig());
    }
    ...
    ****************************************/
    ...
    /**
     * @dev Converts recently accrued swap and redeem fees into fAsset
     * @return mintAmount   fAsset units generated from swap and redeem fees
     * @return newSupply    fAsset total supply after mint
     */
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        ...
        if (surplusFees > 1) {
            ...
            // mint new fAsset to savings manager
     ...
     *      amount of fAsset and sends it over to the SavingsFassetManager.
     * @return mintAmount   fAsset units generated from interest collected from lending markets
     * @return newSupply    fAsset total supply after mint
     */
    function collectPlatformInterest()
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        (uint8[] memory idxs, uint256[] memory gains) = FassetManager.collectPlatformInterest(
            data.bAssetPersonal,
            data.bAssetData
        );

        mintAmount = FassetLogic.computeMintMulti(data.bAssetData, idxs, gains, _getConfig());
    ...
    ****************************************/
     ...
     * @param _cacheSize Maximum percent of total fAsset supply to hold for each bAsset
     */
    ...
    function setTransferFeesFlag(address _bAsset, bool _flag) external override onlyGovernor {
        FassetManager.setTransferFeesFlag(data.bAssetPersonal, bAssetIndexes, _bAsset, _flag);
    }
    ...
    function migrateBassets(address[] calldata _bAssets, address _newIntegration)
        ...
        onlyGovernor
    {
        FassetManager.migrateBassets(data.bAssetPersonal, bAssetIndexes, _bAssets, _newIntegration);
    }
    ...
    function negateIsolation(address _bAsset) external onlyGovernor {
        FassetManager.negateIsolation(data.basket, data.bAssetPersonal, bAssetIndexes, _bAsset);
    }
    ...
    function startRampA(uint256 _targetA, uint256 _rampEndTime) external onlyGovernor {
        FassetManager.startRampA(data.ampData, _targetA, _rampEndTime, _getA(), A_PRECISION);
    }
    ...
    function stopRampA() external onlyGovernor {
        FassetManager.stopRampA(data.ampData, _getA());
    }
    ...
    function mintDeficit() external returns (uint256 mintAmount) {
        ...
        InvariantConfig memory config = _getConfig();
        (, uint256 k) = FassetLogic.computePrice(data.bAssetData, config);
