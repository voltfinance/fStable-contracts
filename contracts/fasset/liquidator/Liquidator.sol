...
contract Liquidator is ILiquidator, Initializable, ModuleKeysStorage, ImmutableModule {
    ...
    address public deprecated_fUSD;
    ...
    ****************************************/
     ...
     * @param _fAsset optional address of the fAsset. eg fUSD or mBTC. Use zero address if from a Feeder Pool.
    ...
    function createLiquidation(
        ...
        if (_fAsset != address(0)) {
            // This Liquidator contract approves the fAsset to transfer bAssets for mint.
            // eg USDC in fUSD or WBTC in mBTC
            IERC20(_bAsset).safeApprove(_fAsset, 0);
            IERC20(_bAsset).safeApprove(_fAsset, type(uint256).max);

            // This Liquidator contract approves the Savings Manager to transfer fAssets
            // for depositLiquidation. eg fUSD
            ...
            IERC20(_fAsset).safeApprove(savings, 0);
            IERC20(_fAsset).safeApprove(savings, type(uint256).max);
        }
    ...
    function reApproveLiquidation(address _integration) external {
        ...
        address fAsset = liquidation.fAsset;
        if (fAsset != address(0)) {
            IERC20(bAsset).safeApprove(fAsset, 0);
            IERC20(bAsset).safeApprove(fAsset, type(uint256).max);

            address savings = _savingsManager();
            IERC20(fAsset).safeApprove(savings, 0);
            IERC20(fAsset).safeApprove(savings, type(uint256).max);
        }
    }
    ...
    ****************************************/
     ...
     *      - COMP for USDC
     *      - ALCX for alUSD
     *    - If bAsset in fAsset. eg USDC in fUSD
     *      - Mint fAsset using bAsset. eg mint fUSD using USDC.
     *      - Deposit fAsset to Savings Manager. eg deposit fUSD
     *    - else bAsset in Feeder Pool. eg alUSD in fPfUSD/alUSD.
    ...
    function triggerLiquidation(address _integration) external override {
        ...
        address fAsset = liquidation.fAsset;
        // If the integration contract is connected to a fAsset like fUSD or mBTC
        if (fAsset != address(0)) {
            // 4a. Mint fAsset using purchased bAsset
            uint256 minted = _mint(bAsset, fAsset);

            // 5a. Send to SavingsManager
            address savings = _savingsManager();
            ISavingsManager(savings).depositLiquidation(fAsset, minted);

            emit Liquidated(sellToken, fAsset, minted, bAsset);
        } else {
            ...
            emit Liquidated(sellToken, fAsset, bAssetBal, bAsset);
        }
    }
     ...
     *        - if from a fAsset
     *          - mints fAssets using bAssets. eg fUSD for USDC
     *          - deposits fAssets to Savings Manager. eg fUSD
    ...
    function triggerLiquidationAave() external override {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // If the integration contract is connected to a fAsset like fUSD or mBTC
            if (fAsset != address(0)) {
                // 5a. Mint fAsset using bAsset from the Uniswap swap
                uint256 minted = _mint(bAsset, fAsset);

                // 6a. Send to SavingsManager to streamed to the savings vault. eg ifUSD or imBTC
                address savings = _savingsManager();
                ISavingsManager(savings).depositLiquidation(fAsset, minted);

                emit Liquidated(aaveToken, fAsset, minted, bAsset);
            } else {
                ...
                emit Liquidated(aaveToken, fAsset, bAssetBal, bAsset);
            }
        }
    ...
    function _mint(address _bAsset, address _fAsset) internal returns (uint256 minted) {
        ...
        minted = IFasset(_fAsset).mint(_bAsset, bAssetBal, minOut, address(this));
    }
}
