 ...
 *          Derived by mStable and adapted for the needs of an fAsset, as described in MIP-7 (http://mips.mstable.org/MIPS/mip-7)
 *          Calculates and validates the result of Fasset operations with respect to the invariant.
...
library FassetLogic {
    ...
    ****************************************/
    ...
    /**
     * @notice Transfers token in, updates internal balances and computes the fAsset output
     * @param _data                 Fasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _input                Data on the bAsset to deposit for the minted fAsset.
     * @param _inputQuantity        Quantity in input token units.
     * @param _minOutputQuantity    Minimum fAsset quantity to be minted. This protects against slippage.
     * @return mintOutput           Quantity of fAsset minted from the deposited bAsset.
     */
    function mint(
        FassetData storage _data,
    ...
    /**
     * @notice Transfers tokens in, updates internal balances and computes the fAsset output.
     * Only fdAsset & fAsset are supported in this path.
     * @param _data                 Fasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _indices              Non-duplicate addresses of the bAssets to deposit for the minted fAsset.
    ...
    function mintMulti(
        ...
        // Transfer the Bassets to the integrator, update storage and calc FassetQ
    ...
    ****************************************/
     ...
     * @return scaledFee          Fee paid, in fAsset terms
     */
    function swap(
        FassetData storage _data,
        InvariantConfig calldata _config,
    ...
    ****************************************/
    ...
    /**
     * @notice Burns a specified quantity of the senders fAsset in return for a bAsset. The output amount is derived
     * from the invariant. Supports redemption into either the fdAsset, fAsset or assets in the fAsset basket.
     * @param _data              Fasset storage state
     * @param _config            Core config for use in the invariant validator
     * @param _output            Data on bAsset to withdraw
     * @param _inputQuantity   Quantity of fAsset to burn
     * @param _minOutputQuantity Minimum bAsset quantity to receive for the burnt fAsset. This protects against slippage.
     * @param _recipient         Address to transfer the withdrawn bAssets to.
     * @return bAssetQuantity    Quanity of bAsset units received for the burnt fAsset
     * @return scaledFee          Fee paid, in fAsset terms
     */
    function redeem(
        FassetData storage _data,
        ...
        // Apply fees, burn fAsset and return bAsset to recipient
        _data.surplus += scaledFee;
     ...
     * balance levels and desired fAsset quantity. Burns the fAsset as payment. Only fdAsset & fAsset are supported in this path.
     * @param _data                 Fasset storage state
     * @param _config               Core config for use in the invariant validator
     * @param _inputQuantity        Quantity of fAsset to redeem
     * @param _minOutputQuantities  Min units of output to receive
     * @param _recipient            Address to credit the withdrawn bAssets
     * @return scaledFee            Fee collected in fAsset terms
        ...
        )
    {
        ...
        // Calc cache and total fAsset circulating
        uint256 maxCache = _getCacheDetails(_data, _config.supply - _inputQuantity + scaledFee);
     ...
     *      relative fAsset quantity from the sender. Only fdAsset & fAsset (0,1) are supported in this path.
     * @param _data                 Fasset storage state
     * @param _config               Core config for use in the invariant validator
     ...
     * @return fAssetQuantity      Quantity of fAsset units to burn as payment
     * @return fee             Fee collected, in fAsset terms
     */
    function redeemExactBassets(
        ...
        address _recipient
    ) external returns (uint256 fAssetQuantity, uint256 fee) {
        // Load bAsset data from storage to memory
        BassetData[] memory cachedBassetData = _data.bAssetData;

        (fAssetQuantity, fee) = computeRedeemExact(
            cachedBassetData,
            _indices,
            _outputQuantities,
            _config,
        ...
        require(fAssetQuantity <= _maxFassetQuantity, "Redeem fAsset qty > max quantity");
        // Apply fees, burn fAsset and return bAsset to recipient
        _data.surplus += fee;
        // Transfer the Bassets to the recipient and count fees
        uint256 maxCache = _getCacheDetails(_data, _config.supply - fAssetQuantity + fee);
    ...
    ****************************************/
    ...
    /**
     * @dev Gets the max cache size, given the supply of fAsset
     * @return maxCache    Max units of any given bAsset that should be held in the cache
     */
    function _getCacheDetails(FassetData storage _data, uint256 _supply)
        internal
        view
    ...
    ****************************************/
    ...
    /**
     * @notice Compute the amount of fAsset received for minting
     ...
     * @return mintAmount   Quantity of fAssets minted
     */
     ...
     * @return scaledSwapFee          Swap fee collected, in fAsset terms
     */
     ...
     * redeeming `quantity` amount of fAsset.
     * @param _bAssets              Array of all bAsset Data
     * @param _o                    Index of output bAsset
     * @param _grossFassetQuantity  Net amount of fAsset to redeem
     * @param _config               Generalised invariantConfig stored externally
    ...
    function computeRedeem(
        ...
        uint256 k0 = _invariant(x, sum, _config.a);
        uint256 redemption;
        (redemption, scaledFee) = _getFee(_grossFassetQuantity, _config, _feeRate, k0);
    ...
    function _getFee(
        ...
        redemption = _grossFassetQuantity;
        if (_config.supply > _k0) {
    ...
    /**
     * @notice Compute the amount of fAsset required to redeem
     * a given selection of bAssets.
     * @param _bAssets          Array of all bAsset Data
     * @param _indices          Indexes of output bAssets
     * @param _rawOutputs       Desired raw bAsset outputs
     * @param _config           Generalised invariantConfig stored externally
     * @return grossFasset      Amount of fAsset required to redeem bAssets
    ...
    function computeRedeemExact(
        ...
        // 5. Total fAsset is the difference between values
        uint256 redeemed = (_config.supply * (k0 - k1)) / k0;
        require(redeemed > 1e6, "Must redeem > 1e6 units");
        grossFasset = redeemed.divPrecisely(1e18 - _feeRate);
        fee = grossFasset - redeemed;
        grossFasset += 1;
        if (_config.supply > k0) {
            grossFasset = ((grossFasset * 1e18) / (1e18 - _config.recolFee));
        }
    }

    /**
     * @notice Gets the price of the fAsset, and invariant value k
     * @param _bAssets  Array of all bAsset Data
     * @param _config   Generalised InvariantConfig stored externally
     * @return price    Price of an fAsset
    ...
    ****************************************/
     ...
     * @param _config       Generalised InvariantConfig stored externally
     * @return mintAmount   Amount of value added to invariant, in fAsset terms
     */
